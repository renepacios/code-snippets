...
/*
	Revisiones:
		- 18-09-2013 - Agregada funciones obtner
					 - Agregada linea para que implemente el interfaz de los DAOS
...
set @VOName='tmpIrisIdentVO'
SET @DAOName='tmpIrisIdentDAO' 
...
--PRINT '		Inherits BaseDAO(Of '+ @DAOName+')
print'	Implements IDAO(Of '+@VOName+')';
print '';
...
PRINT 'Public Sub Insertar(cn As SqlConnection, VO As '+@VOName+',Optional configWeb As ConfigWebVO = Nothing, Optional tran As SqlTransaction = Nothing) Implements IDAOContract(Of '+@VOName+').Insertar'
print '		''quitar campos clave de la consulta'
...
PRINT '            VO.'+@firstCampoVO+' = CType(cmd.ExecuteScalar(), Integer)'
PRINT '        End Using'
PRINT ''
PRINT 'End Sub'
...
PRINT 'Public Sub Actualizar(cn As SqlConnection, VO As '+@VOName+',Optional configWeb As ConfigWebVO = Nothing, Optional tran As SqlTransaction = Nothing)  Implements IDAOContract(Of '+@VOName+').Actualizar'
print ''
print '		''quitar campos clave de la consulta'
...
PRINT 'End Sub'
	PRINT '';	
	print'
   Public Sub Borrar(cn As SqlConnection, id As Integer,Optional configWeb As ConfigWebVO = Nothing, Optional tran As SqlTransaction = Nothing) Implements IDAOContract(Of '+@VOName+').Borrar
        Const strSQL As String = " DELETE FROM [dbo].['+@TableName+'] " & _
                                 	" WHERE '+@firstCampo+'=@'+@firstCampoVO+' " ''falta implementar la clave 

        Using cmd As New SqlCommand(strSQL, cn, tran)		   
            cmd.Parameters.AddWithValue("@'+@firstCampoVO+'", id)
            cmd.ExecuteNonQuery()
        End Using
    End Sub
'
...
print 'Public Function Obtener(cn As SqlConnection, cantidadDeRegistros As Integer, pagina As Integer,Optional configWeb As ConfigWebVO = Nothing, Optional tran As SqlTransaction = Nothing) As IEnumerable(Of  '+@VOName+') Implements IDAOContract(Of  '+@VOName+').Obtener'
print ''
print '			Const criterioOrdenacion As String = " Order by t.'+@firstCampo+' Desc"	    '
print ''
PRINT '			Dim strSQL As String = " SELECT '+@FIELDS+'  " + _ '
...
print 'Public Function Obtener(cn As SqlConnection, id As Integer,Optional configWeb As ConfigWebVO = Nothing, Optional tran As SqlTransaction = Nothing) As '+@VOName+' Implements IDAOContract(Of '+@VOName+').Obtener														'
print ''
PRINT '			Const strSQL As String = " SELECT '+@FIELDS+'  " + _ '
print '									" FROM '+@TableName+' t " + _ '
PRINT '									" WHERE '+@firstCampo+'=@'+@firstCampoVO+' " ''falta implementar la clave '
print '			'
print '   
        Dim VODev As '+@VOName+' = Nothing

        Using cmd As New SqlCommand(strSQL, cn, tran)
            cmd.Parameters.AddWithValue("@'+@firstCampoVO+'", id)
            Using dr As IDataReader = cmd.ExecuteReader()
                If dr.Read() Then VODev = ObtenerVO(dr)
            End Using
        End Using
        Return VODev '
...
DECLARE @AUX_STR VARCHAR(200)	
	...
	FETCH FIRST FROM DeclarationCursor 
	INTO @ColumnName, @DataType,@IsNullable,@intDateType;
	PRINT '#Region "Funciones privadas"';
